...
def test_megkde_1d_changing_weights(rng):
    # Draw from normal, fit KDE, see if sampling from kde's pdf recovers norm
...
def test_megkde_2d_basic(rng):
    ...
    cs_y /= cs_y[-1]
    cs_y[0] = 0
    samps_x = interp1d(cs_x, xs)(rng.uniform(size=10000))
    samps_y = interp1d(cs_y, ys)(rng.uniform(size=10000))
