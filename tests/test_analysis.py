...
class TestChain:
    ...
    def test_hdi_degenerate(self):
        df = pd.DataFrame({"x": [4.2], "weight": [2.0]})
        chain = Chain(samples=df, name="degenerate", statistics=SummaryStatistic.HDI)
        consumer = ChainConsumer()
        consumer.add_chain(chain)

        bound = consumer.analysis.get_summary()["degenerate"]["x"]
        assert np.isclose(bound.lower, 4.2)
        assert np.isclose(bound.center, 4.2)
        assert np.isclose(bound.upper, 4.2)
    ...
    def test_hdi_intervals_multimodal(self):
        ...
        single_interval = consumer.analysis.get_parameter_hdi_intervals(chain_default, "x")
        assert len(single_interval) == 1
    ...
    def test_multimodal_hdi_intervals_respect_target_mass(self):
        rng = np.random.default_rng(123)
        broad = rng.normal(loc=0.0, scale=3.0, size=8000)
        narrow = rng.normal(loc=2.5, scale=0.05, size=400)
        df = pd.DataFrame({"x": np.concatenate((broad, narrow))})
        chain = Chain(
            samples=df,
            name="multimass",
            statistics=SummaryStatistic.HDI,
        ...
        xs, ys, _ = consumer.analysis._get_smoothed_histogram(chain, "x", pad=True, use_kde=False)
        ys = ys / np.trapezoid(ys, xs)
        mask = np.zeros_like(xs, dtype=bool)
        for lower, upper in intervals:
            mask |= (xs >= lower) & (xs <= upper)
        mass = np.trapezoid(np.where(mask, ys, 0.0), xs)

        assert np.isclose(mass, chain.summary_area, atol=2e-2)
        assert any(lower > 1.8 for lower, _ in intervals)
...
class TestHDIParityWithArviz:
    ...
    def test_hdi_matches_arviz_low_sample_unimodal(self) -> None:
        rng = np.random.default_rng(12345)
        samples = rng.normal(loc=0.5, scale=1.1, size=25)
        area = 0.5
    ...
    def test_hdi_matches_arviz_high_sample_unimodal(self) -> None:
    ...
    def test_hdi_matches_arviz_low_sample_multimodal(self) -> None:
        rng = np.random.default_rng(10)
        samples = np.concatenate([rng.normal(-1.5, 0.15, size=60), rng.normal(1.4, 0.2, size=60)])
        area = 0.5

        consumer, chain = self._make_chain(samples, area=area, multimodal=True, smooth=2)
        ...
        np.testing.assert_allclose(cc_intervals, az_intervals, atol=2e-1)

    def test_hdi_matches_arviz_high_sample_multimodal(self) -> None:
        rng = np.random.default_rng(0)
        samples = np.concatenate([rng.normal(-2.0, 0.3, size=2000), rng.normal(2.0, 0.25, size=2000)])
        area = 0.5
